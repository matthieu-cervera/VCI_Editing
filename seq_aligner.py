'''
heavily inspired by https://github.com/sled-group/InfEdit/blob/main/seq_aligner.py

seq_aligner aims to build a mapper between a source prompt and a target prompt for attention control purposes
'''
...
def get_mapper(x: str, y: str, specifier, tokenizer, encoder, device, encoder_type, max_len=77):        # encoder type : FLAN or CLAP
    ...
    if encoder_type=="FLAN":
    ...
    elif encoder_type=="CLAP_AudioLDM2":
        x_encoded = tokenizer(
                        x,
                        padding="max_length",
                        max_length=max_len,
        ...
        x_output = encoder.text_model(input_ids=x_encoded) 
        y_output = encoder.text_model(input_ids=y_encoded)
        x_latent = encoder.text_projection(x_output.last_hidden_state).squeeze(0)  
        y_latent = encoder.text_projection(y_output.last_hidden_state).squeeze(0)


    else:       # CLAP encoder and tokenizer
        x_encoded = tokenizer(
                        x,
                        padding="max_length",
                        max_length=max_len,
        ...
        x_output = encoder.base(input_ids=x_encoded) 
        y_output = encoder.base(input_ids=y_encoded)
        x_latent = encoder.projection(x_output.last_hidden_state).squeeze(0)  
        y_latent = encoder.projection(y_output.last_hidden_state).squeeze(0)
    ...
    while (i < len(y_seq)-1) and (j < len(m_seq)-1):
      ...
      if m_seq[j] == x_seq[mapper[i]]:
        ...
        print(x, y, local_prompt, mutual_prompt)
        raise ValueError("mutual prompt not found in target prompt")

    return mapper, alphas, m, alpha_e, alpha_m


def get_refinement_mapper(prompts, specifiers, tokenizer, encoder, device, encoder_type="FLAN", max_len=77):
    x_seq = prompts[0]
    mappers, alphas, ms, alpha_objs, alpha_descs = [], [], [], [], []
    for i in range(1, len(prompts)):
        mapper, alpha, m, alpha_obj, alpha_desc = get_mapper(x_seq, prompts[i], specifiers[i-1], tokenizer, encoder, device,  encoder_type, max_len)
