...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):# x:(b,T,C), context:(b,seq_len,context_dim)
        ...
        # print(f'attn shape before processing : {attn.shape}')
        # print(f'qkv shapes before processing: {q.shape, k.shape, v.shape}')

        # MATT2
        if self.controller is not None:
            if self.controller.is_matt and self.controller.attention_pass: # new class
                if self.controller.is_target_only:
                    # attn_slow = self.controller.new_refine_crossattention(attn)
                    attn = self.controller.new_refine_crossattention_light(attn)

                else:
                    q,k,v = self.controller.new_forward(q,k,v, self.scale)
                    sim = einsum('b i d, b j d -> b i j', q, k) * self.scale
                    attn = sim.softmax(dim=-1)
            else:
                if self.controller is not None:   # here controller is of class AttentionRefine
                    if is_cross:
                        print(f"crossattention ! / {self.place_in_unet}")
                        attn = self.controller.forward_ca(attn, is_cross, self.place_in_unet)
                    else:
                        q,k,v = self.controller.forward(q,k,v,h)        # self att forward
                        sim = einsum('b i d, b j d -> b i j', q, k) * self.scale # (b*head,T,seq_len)
                        attn = sim.softmax(dim=-1)
